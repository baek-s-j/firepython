...
from PyQt5.QtWidgets import QWidget, QHBoxLayout, QListWidget, QLabel, QPushButton, QVBoxLayout, QDialog, \
...
# 지도 뷰에서 좌표 데이터를 받아옴.
# 아케이트 형식에 맞게 데이터를 입력받음.

class ArcadeEditor(QWidget):
    def __init__(self, arcade):
        ...
        btn.clicked.connect(lambda: self.addArcade(state='cur'))

        btn2 = QPushButton('끝에 추가', self)
        btn2.setFont(font)
        btn2.setFixedSize(150, 45)
        btn2.clicked.connect(lambda: self.addArcade(state='end'))
    ...
    def addArcade(self, state):
        ...
        if self.dialog.exec():
            ...
            if state == 'cur':
                ...
                self.arcade.insert(self.list_widget.currentRow(), tmp)
            else:
                self.arcade.append(tmp)
        self.syncListWidget()

    def syncListWidget(self):
        self.list_widget.clear()
        for i in range(len(self.arcade)):
            self.list_widget.addItem(str(i + 1) + '번: 건물번호 ' + self.arcade[i]['id'] + '\n세부사항: ' + self.arcade[i]['detail'])
        self.list_widget.setCurrentItem(self.list_widget.item(0))

    def delete(self):
        del self.arcade[self.list_widget.currentRow()]
        self.syncListWidget()
...
class InputDialog(QDialog):
    def __init__(self, parent=None):
        ...
        layout.addRow("건물 번호 (예시 1-3-2)", self.id)
        layout.addRow("옥상 입구 번호", self.num)
        layout.addRow("세부사항", self.detail)
    ...
    def getInputs(self):
        arcade = {'id': self.id.text(), 'num': self.num.text(), 'detail': self.detail.text()}
        return arcade
